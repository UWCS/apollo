...
"""
ADDF_LONG_TEXT = """
Adds a filament type to the bot's database. Format is a name in quotes, a cost per kilo, and an image of the filament (attached to the message or linked)
"""
DELF_LONG_TEXT = """
...
LIST_HELP_TEXT = """Lists all of the available filament. Takes an optional list filter"""
...
class PrintTools(commands.Cog, name="Print tools"):
    ...
    @printtools.command(help=ADDF_HELP_TEXT, brief=ADDF_HELP_TEXT)
    @is_compsoc_exec_in_guild()
    async def addf(self, ctx: Context, *args: clean_content):
        # Check we have the bare minumum number of args
        if len(args) < 2:
            await ctx.send("I need at least a filament name in quotes and its cost per kilogram.")
            return

        # Make sure there are the right number of args for each input type
        if len(args) < 3 and not ctx.message.attachments:
            await ctx.send("Please provide an image of the filament by an image link or attachment")
        ...
        filament = FilamentType(name=filament_name, cost=filament_cost, image_path=str(image_file))
    ...
    @is_compsoc_exec_in_guild()
    async def delf(self, ctx: Context, filament_name: clean_content):
        filament = db_session.query(FilamentType).filter(FilamentType.name.like(filament_name)).first()

        if not filament:
            await ctx.send(f"Couldn't find a filament that matches the name \"{filament_name}\"")
            return
    ...
    async def listf(self, ctx: Context, *filter: clean_content):
        if filter:
            ...
            filaments = db_session.query(FilamentType)\
        ...
        if filaments:
            filament_list = '\n'.join(map(lambda x: f" • **{x.name}** - £{x.cost}/kg", filaments))
