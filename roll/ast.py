...
def isfunction(token: Any) -> TypeGuard["TokenFunction"]:
...
class Environment:
    def __init__(self, root: Any) -> None:
        self.root = root
        self.closure: dict["int | IToken", "IToken"] = {}
        self.trace: list["IToken | str"] = []

    def copy(self) -> "Environment":
        out = Environment(self.root)
        out.closure = self.closure.copy()
        out.trace = self.trace.copy()
        return out
...
class IToken(ABC):
    @trace
    @abstractmethod
    def reduce(self, env: Environment, counter: "HashCounter") -> Any:
        """Returns a fully reduced version of the token"""
        raise NotImplementedError

    @abstractmethod
    def substitute(self, old_to_new: dict["int | IToken", "int | IToken"]) -> Any:
    ...
    def dereference(self, env: Environment) -> Any:
        """Attempts to deference the token if it is a pointer"""
        return self

    def rolls(self, env: Environment) -> int:
        """Returns the total number of roll operations in this token subtree."""
        return 0

    @trace
    @abstractmethod
    def hash_vars(self, *_) -> Any:
...
class Assignment:
    def __init__(self, name: str, expression: IToken, id: int | None = None) -> None:
        self.name: str = name
        self.expression: IToken = expression
        self.identifier: int = id if id is not None else hash(name)
...
class TokenNumber(IToken, IPure):
    ...
    @trace
    def reduce(self, *_) -> "TokenNumber":
        return self

    def substitute(self, old_to_new: dict[int | IToken, int | IToken]) -> "TokenNumber":
        return TokenNumber(self.__value)

    @trace
    def hash_vars(self, *_) -> None:
        pass
...
class TokenString(IToken, IPure):
    ...
    @trace
    def reduce(self, *_) -> "TokenString":
        return self

    def substitute(self, old_to_new: dict[int | IToken, int | IToken]) -> "TokenString":
...
class TokenRoll(IToken):
    def __init__(self, count: IToken, sides: IToken) -> None:
        self.count = count
        self.sides = sides
        logging.debug(self.__class__.__name__, self)

    @trace
    def reduce(self, env: Environment, counter: HashCounter) -> TokenNumber:
    ...
    def substitute(self, old_to_new: dict[int | IToken, int | IToken]) -> "TokenRoll":
        return TokenRoll(
            self.count.substitute(old_to_new), self.sides.substitute(old_to_new)
        )

    @trace
    def hash_vars(self, counter: HashCounter, map: dict[str, int]) -> None:
...
class TokenVariable(IToken):
    def __init__(self, name: str, id: IToken | int | None = None) -> None:
        self.name = name
        self.identifier: int | IToken = hash(name) if id is None else id
        logging.debug(self.__class__.__name__, self)

    @trace
    def reduce(self, env: Environment, counter: HashCounter) -> IToken:
        return self.dereference(env).reduce(env, counter)

    def substitute(self, old_to_new: dict[int | IToken, int | IToken]) -> "TokenVariable":
    ...
    def dereference(self, env: Environment) -> IToken:
        id = self.identifier
        try:
...
class TokenLet(IToken):
    ...
    def __init__(self, declarations: list[Assignment], expression: IToken) -> None:
        self.declarations: list[Assignment] = declarations
        self.expression: IToken = expression
        logging.debug(self.__class__.__name__, self)
    ...
    def substitute(self, old_to_new: dict[int | IToken, int | IToken]) -> "TokenLet":
        ...
        # Copy the let body
        new_expr: IToken = self.expression.substitute(old_to_new)
    ...
    def update_env(self, env: Environment) -> Environment:
        new_env = env.copy()
        for decl in self.declarations:
...
class TokenFunction(IToken):
    def __init__(self, arg_name: str, expression: "TokenFunction | TokenLet", argid: int | None = None) -> None:
        self.arg_name: str = arg_name
        self.arg_id: int = argid if argid is not None else hash(arg_name)
        self.expression: "TokenFunction | TokenLet" = expression
        logging.debug(self.__class__.__name__, self)

    @trace
    def reduce(self, env: Environment, counter: HashCounter) -> "TokenFunction":
        return self

    def substitute(self, old_to_new: dict[int | IToken, int | IToken]) -> "TokenFunction":
...
class TokenApplication(IToken):
    def __init__(self, lhs: TokenFunction, rhs: list[TokenFunction]) -> None:
        self.lhs = lhs
        self.rhs = rhs
        logging.debug(self.__class__.__name__, self)
    ...
    def reduce(self, env: Environment, counter: HashCounter) -> IToken:
        ...
        out: TokenFunction | TokenLet = self.lhs.dereference(env)
        decls: list[Assignment] = []
        for expr in self.rhs:
            assert isinstance(out, TokenFunction)
        ...
        substitutions: dict[int | IToken, int | IToken] = {}
        env = env.copy()
        for decl in decls:
        ...
        if isfunction(out):
            func_path: list[TokenFunction] = [out]
    ...
    def substitute(self, old_to_new: dict[int | IToken, int | IToken]) -> "TokenApplication":
...
class TokenOperator(IToken):
    mapping: dict[Operator, Callable[[list[Any]], Any]] = {
    ...
    def __init__(self, op: Operator, args: list[IToken]) -> None:
        self.op = op
        self.args = args
        logging.debug(self.__class__.__name__, self)
    ...
    def substitute(self, old_to_new: dict[int | IToken, int | IToken]) -> "TokenOperator":
        new_args: list[IToken] = []
...
class TokenTernary(IToken):
    def __init__(self, condition: IToken, true: IToken, false: IToken) -> None:
    ...
    def substitute(self, old_to_new: dict[int | IToken, int | IToken]) -> "TokenTernary":
...
class TokenCase(IToken):
    def __init__(self, expression: IToken, pairs: list[list[IToken]]) -> None:
    ...
    def substitute(self, old_to_new: dict[int | IToken, int | IToken]) -> "TokenCase":
        new_expr = self.expression.substitute(old_to_new)
        new_pairs: list[list[IToken]] = []
        for pair in self.pairs:
            new_pairs.append(
...
class Program:
    ...
    def __init__(self, expressions: list[Any]) -> None:
        ...
        self.counter: HashCounter = HashCounter()
        # Debug
        logging.debug(self.__class__.__name__, self)
    ...
    @property
    def string_rep(self)  -> "StringRep":
        return self.StringRep(self.expressions, self.assignments)

    class StringRep:
        def __init__(self, expressions: list[Any], assignments: list[Assignment]) -> None:
