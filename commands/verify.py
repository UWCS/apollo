...
async def verify(ctx: Context, uni_number: str):
    ...
    api_request = requests.get('https://uwcs.co.uk/accounts/api/{uni_id}/'.format(uni_id=uni_number), headers=headers)
    ...
    if api_request.status_code == 200:
        ...
        if not api_username:
            ...
            try:
                compsoc_role = [role for role in compsoc_guild.roles if role.name == CONFIG['UWCS_MEMBER_ROLE_NAME']][0]
            except IndexError:
                raise VerifyError(
...
@verify.error
async def verify_error_handler(ctx: Context, error: VerifyError):
