...
def test_single_not_in_blacklist(database):
    assert parse_message('foo++', database) == [RawKarma(name='foo', op='++', reason=None)]
...
def test_blacklist_single_allowed_lower(database):
    assert parse_message('"c"++', database) == [RawKarma(name='c', op='++', reason=None)]
...
def test_blacklist_single_allowed_mixed(database):
    assert parse_message('"NoTepAD"++', database) == [RawKarma(name='NoTepAD', op='++', reason=None)]
...
def test_multiple_not_in_blacklist(database):
    assert parse_message('foo++ bar++', database) == [RawKarma(name='foo', op='++', reason=None),
...
def test_blacklist_multiple_allowed(database):
    assert parse_message('"c"++ "notepad"++', database) == [RawKarma(name='c', op='++', reason=None),
...
def test_blacklist_mixed_quoted_allowed(database):
    assert parse_message('"c++" "notepad"++', database) == [RawKarma(name='notepad', op='++', reason=None)]
