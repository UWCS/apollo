...
class RoomSearch(commands.Cog):
    ...
    async def room(self, ctx: Context, name: str):
        ...
        if not rooms: return await ctx.reply("Room does not exist. Ensure you are giving the full name")

        if len(rooms) > 1:
            room = await self.choose_room(ctx, rooms)
            if room is None: return
        else: room = rooms[0]   # Only one in rooms
        ...
        img = discord.File(self.req_img("https://search.warwick.ac.uk/api/map-thumbnail/" + str(room["w2gid"])), filename="map.png")
    ...
    async def choose_room(self, ctx, rooms):
        ...
        rooms_text = "".join(f"\n\t{e} {r['name']} in **{r['building']}** {r['floor']}" for r, e in zip(rooms, emojis))
        conf_message = await ctx.send(header + rooms_text)
        for e in emojis:
            await conf_message.add_reaction(e)

        try:
            check = lambda r, u: r.message.id == conf_message.id and u == ctx.message.author and str(r.emoji) in emojis
            react_emoji, _ = await ctx.bot.wait_for("reaction_add", check=check, timeout=60)
        except TimeoutError:
            return None
    ...
    def get_room_infos(self, room):
        ...
        if map_req is None or map_req["total"] == 0: return []
